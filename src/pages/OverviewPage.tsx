...
const OverviewPage = () => {
  ...
  const [filters, setFilters] = useState<any>({ numericFilters: {}, categoricalFilters: {} });
  ...
  const handleSelectDataset = (selectedDatasetId: string) => {
    setFilters({ numericFilters: {}, categoricalFilters: {} });
  ...
  const fetchDatasetOverview = async () => {
    try {
      setLoading(true);
      const hasActiveFilters = Object.keys(filters.numericFilters).length > 0 || 
        Object.values(filters.categoricalFilters).flat().length > 0;
      
      toast.loading(hasActiveFilters ? "Analyzing filtered data with AI..." : "Analyzing dataset with AI...");

      // Call AI-powered overview generation
      const { data: overviewData, error: overviewError } = await supabase.functions.invoke(
        "generate-overview",
        { body: { datasetId, filters } }
      );
      ...
      setStatistics(stats);
      toast.dismiss();
      const filterMsg = Object.keys(filters.numericFilters).length > 0 || 
        Object.values(filters.categoricalFilters).flat().length > 0 
        ? " (with filters applied)" : "";
      toast.success(`Dataset analysis completed${filterMsg}!`);
    } catch (error) {
